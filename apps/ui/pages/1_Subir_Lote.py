...
from utils.api_client import ApiClientError, create_batch, run_batch
...
st.caption(
    "Usa formato Python con llaves, p.ej: `Proceso_{NumeroProceso}_{NombreContribuyente}`. "
    "Tambien existe `{row_index}`."
)
...
if st.session_state.batch_id:
    ...
    if st.button("Procesar lote"):
        try:
            ...
            st.success("Procesamiento iniciado. Ve a la pagina **Progreso**.")
        except ApiClientError as e:
            st.error(str(e))
